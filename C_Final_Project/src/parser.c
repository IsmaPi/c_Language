    ...
    switch (parser->current_token->type) {
        case TOKEN_INT:
            // Handle integer literals
            {
                ...
                node->value.int_value = atoi(parser->current_token->value); // Convert string to int
        ...
        case TOKEN_FLOAT:
            ...
            {
                AST_T* node = init_ast(AST_LITERAL);
                node->value.float_value = atof(parser->current_token->value); // Convert string to float
...
AST_T* parse_integration_or_derivation(parser_T* parser) {
    if (parser->current_token->type == TOKEN_INTEGRATE || parser->current_token->type == TOKEN_DERIVATE) {
        ...
        // Corrected from parser_expression(parser) to parser_parse_expr(parser, parser->scope)
        AST_T* expr = parser_parse_expr(parser, parser->scope); // Parse the expression inside
